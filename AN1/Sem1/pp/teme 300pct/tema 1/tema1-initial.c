...
int a[1000][1000],b[1000][1000],i,j,n,m; //a matricea initiala,b copia sa dupa evolutie
                                         //i si j contori
                                         //n nr de linii,m nr de coloane

void citire(int n,int m){//citirea configuratiei initiale de la care porneste evolutia
    for(i=1;i<=n;i++)
        for(j=1;j<=m;j++)
            scanf("%d",&a[i][j]);
}
...
int vecini_toroid(int i,int j){//cautam vecinii celulei parcurgand matricea circular
...
void copiere(int n,int m){//folosim o copie a matricei pentru a putea
    for(i=1;i<=n;i++)     //pastra evolutia de la pasul anterior
        for(j=1;j<=m;j++)
            a[i][j]=b[i][j];
}
...
void afisare(int n,int m){ //afisarea evolutiei
    ...
    printf("\n%.3f%%",(nr*100)/(n*m));//pentru a scrie '%' folosim "%%" sau printf("%c", '%');
}

int main(){
    int k,p; //k este numarul de etape,p este contorul etapelor
    char tip;
            ...
            if(n<3||m<3)//o matrice cu <3 linii sau coloane
                ...
                for(p=1;p<=k;p++){
                    for(i=1;i<=n;i++)
                        for(j=1;j<=m;j++)
                            if(a[i][j]){
                                if(vecini_toroid(i,j)<2)
                                    b[i][j]=0;
                                else
                                    if(vecini_toroid(i,j)<=3)
                                        b[i][j]=1;
                                    else b[i][j]=0;
                            }
                            else if(vecini_toroid(i,j)==3)
                                    b[i][j]=1;
                                else b[i][j]=0;
                    copiere(n,m);
                }
