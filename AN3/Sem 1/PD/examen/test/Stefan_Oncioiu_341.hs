...
getNoStr (x:xs) = if(isDigit x) then (x: getNoStr xs) else []
...
getNo :: String -> Int
getNo x = if(length x == 0) then [] else 1

--numar :: String -> Int
--numar x = 10^length x +map (^ 10) x
...
removeNotDigit x = (filter isAlpha x)
...
parseTerm x = if (head x == '+') 
              then 0 
              else if (head x == '-')
              then 1
              else 2

--numberr :: String -> Integer
--number [] = []
--numberr (x:xs) =
...
isPlus :: Char -> Bool
isPlus s = if (s=='+') then True else False
